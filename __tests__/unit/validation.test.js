...
test('Validate by module10 --Success Case', () => {
    const result = validations.ValidationByModule10({ value: mock.firstModule, digit: mock.firstModuleDigit, initialDigit: 2 })
...
test('Validate by module10 DV --Success Case', () => {
    const result = validations.validationConsumptionAccountsByModule10({ barCode: mock.barcode10 })
    expect(result).toBe(true)
})

test('Validate by module10 DV --Success Case', () => {
    const result = validations.validationInitialDigitsConsumptionAccounts({ barCode: mock.firstModuleConsume, rulesByDigit: mock.rulesByDigit })

...
test('Function normalizeReturnByValidation --Success Case', () => {
  ...
  expect(result).toBe(
    'Err-101.3: A primeira parte do seu código apresenta erro na validação \n'
  )
})

test('Function validateAndFormatDate --Success Case', () => {
  const result = normalizes.removeOnePositionInString({
    string: '0123X56789',
    position: 4,
  })
...
test('Function getBarcode44Positions --Success Case', () => {
  const result = normalizes.getBarcode44Positions({
    barCodeCutInArray: barcodeArrayConsume,
...
test('Function getBarcode44Positions --Success Case', () => {
  const result = normalizes.getBarcode44Positions({
    barCodeCutInArray: barcodeArrayBillet,
  })

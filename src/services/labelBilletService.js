...
function validationByModule({ barCodeCutInArray, positionArray, initialDigit }) {
...
function getValidationByDV({ barCodeCutInArray, type }) {
    ...
    if (type == "consumptionAccounts") {
        ...
        if (['8', '9'].includes(barCodeCutInArray[0][2])) {
            ...
            validationResult = utils.validationConsumptionAccountsByModule10({ barCode })
        }
...
function getBarCodeCutInArray({ barCode, modules }) {
    ...
    modulesToArray.forEach((module) => {
        if (module[1].value) array.push(barCode.slice(module[1].value.initial, module[1].value.end))
        if (module[1].digit) array.push(barCode[module[1].digit])
    })
    return array
}

function getValidationByModule({ barCodeCutInArray, validationModule, rulesByModule }) {
    return validationModule.map((module, index) => {
        const { ruleModule, initialDigit } = rulesByModule[module] || {}
        const validationResult = validationByModule({ ruleModule, barCodeCutInArray, positionArray: index, initialDigit })
...
module.exports = {
    getRulesLabelBillet({ barCode }) {
        if (!barCode) throw new BadRequestException('Err-001: Favor enviar um código de barras.')
        if (barCode.length == 48 && '8' === barCode[0]) {
            return 'consumptionAccounts'
        } else if (barCode.length == 47) {
            return 'billingAccounts'
        }
        throw new BadRequestException('Err-002: Tamanho do código de barras invalido. Favor conferir a linha digitavel')
    },

    validateDigitsBarCode({ barCode, rules, type }) {

        const barCodeCutInArray = getBarCodeCutInArray({ barCode, modules: rules.modules })
        const validationModule = getValidationByModule({ barCodeCutInArray, type, validationModule: rules.validationByModule, rulesByModule: rules.modules })
        if (type == "consumptionAccounts") {
            const validationInitialDigits = utils.validationInitialDigitsConsumptionAccounts({ barCode: barCodeCutInArray[0], rulesByDigit: rules.rulesByDigit })
            validationModule.push(...validationInitialDigits)
        }

        const validationDV = getValidationByDV({ barCodeCutInArray, type })
        const fails = normalizes.normalizeReturnByValidation(validationModule.concat(validationDV))
    ...
    getExpirationDate({ barCode, type }) {
        if (type === 'consumptionAccounts') {
            ...
            if (!cutDueDate) {
                let cutDate = normalizes.removeOnePositionInString({ string: barCode.slice(20, 29), position: 3 })
    ...
    getAmount({ barCode, type }) {
        if (type === 'consumptionAccounts') {
            let cutAmount = normalizes.removeOnePositionInString({ string: barCode.slice(4, 16), position: 7 })

...
module.exports = {
  ...
  async validate(request, response) {
    try {
      ...
      const barCodeFormatted = labelBilletService.validateDigitsBarCode({
        barCode,
        rules: rules[type],
        type,
      })
      const expirationDate = labelBilletService.getExpirationDate({
        barCode,
        type,
      })
      ...
      return response
        .status(200)
        .json({ barCode: barCodeFormatted, amount, expirationDate })
    } catch (error) {

...
module.exports = {
    ...
    ValidationByModule10({ value, digit, initialDigit }) {
        ...
        const sumBarCode = arrayBarCodeMultiples.reduce((previousValue, currentValue) => previousValue + currentValue)
    ...
    ValidationByModule11({ barCode, type }) {
        ...
        const [verifyDigit] = isConsumptionAccounts ? barCodeToArray.splice(3, 1) : barCodeToArray.splice(4, 1)
        ...
        const sumBarCode = multipliersBarcodeResult.reduce((previousValue, currentValue) => previousValue + currentValue)
    ...
    validationInitialDigitsConsumptionAccounts({ barCode, rulesByDigit }) {
        ...
        for (const ruleDigit in rulesByDigit) {
            const isValid = rulesByDigit[ruleDigit].includes(Number(barCodeToArray[ruleDigit - 1]))

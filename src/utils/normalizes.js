...
module.exports = {
  ...
  factoryDueData(factoryDueData) {
    ...
    if (Number(factoryDueData) > 1000)
      return new Date(
        initialDate.setDate(initialDate.getDate() + factoryDueData)
  ...
  validateAndFormatDate(date) {
    ...
    if (
      nowDate.getFullYear() <= dateFormatted.getFullYear() &&
  ...
  normalizeReturnByValidation(validation) {
    ...
    validation.map((data) => {
      if (!data.isValid) {
        fails += `Err-10${data.module}: ${
          dictionary[data.module]
        } apresenta erro na validação \n`
      }
    })
    return fails
  },
